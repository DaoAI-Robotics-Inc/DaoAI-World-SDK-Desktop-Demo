...
int main()
{
    ...
    std::string image_path = rootpath + "output/object_detection_img.png";   //ͼ��·��
    std::string model_path = rootpath + "output/object_detection_model.dwm";    //ģ��·��
    ...
    try
    {
        ...
        std::ofstream fout(rootpath + "output/testObjectDetection_Result.json");
        fout << prediction.toJSONString() << "\n";
        fout.close();

        //���ͼ��д��
        std::cout << "write result image" << std::endl;
        resultImage.save(rootpath + "output/testObjectDetection_Result.bmp");
