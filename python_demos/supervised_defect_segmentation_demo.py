...
def load_model(model_path, device=dlsdk.DeviceType.CPU):
    ...
    try:
        ...
        model = dlsdk.SupervisedDefectSegmentation(model_path, device=device)
...
def run_inference(model, daoai_image, confidence_threshold=0.95):
    ...
    Args:
        model: The supervised defect segmentation model object.
...
def print_detection_results(prediction, max_points_to_print=3):
    ...
    logger.info("\nMasks to Polygons:")
    for key in prediction.masks.keys():
        polygons = prediction.masks[key].toPolygons()
        for i, polygon in enumerate(polygons):
            logger.info(f"  {key} Polygon {i + 1}:")
...
def generate_output_paths(base_dir=r"python_demos\output"):
    ...
    image_output_path = os.path.join(image_output_dir, f"supervised_defect_segmentation_result_{timestamp}.png")
    json_output_path = os.path.join(json_output_dir, f"supervised_defect_segmentation_prediction_{timestamp}.json")
    annotation_output_path = os.path.join(json_output_dir, f"supervised_defect_segmentation_annotation_{timestamp}.json")
    
    return image_output_path, json_output_path, annotation_output_path

def main():
    """Main function to demonstrate supervised defect segmentation."""
    logger.info("=== Starting Supervised Defect Segmentation Demo ===\n")
    
    # Paths (update these to your environment)
    model_path = r"python_demos\data\supervised_defect_segmentation_model.dwm"
    image_path =  r"python_demos\data\supervised_defect_segmentation_img.png"
    ...
    logger.info("=== Supervised Defect Segmentation Demo Completed ===")
