...
def load_model(model_path, device=dlsdk.DeviceType.CPU):
    ...
    try:
        ...
        model = dlsdk.InstanceSegmentation(model_path, device=device)
...
def run_inference(model, daoai_image, confidence_threshold=0.95):
    ...
    Args:
        model: The instance segmentation model object.
...
def generate_output_paths(base_dir=r"python_demos\output"):
    ...
    image_output_path = os.path.join(image_output_dir, f"instance_segmentation_result_{timestamp}.png")
    json_output_path = os.path.join(json_output_dir, f"instance_segmentation_prediction_{timestamp}.json")
    annotation_output_path = os.path.join(json_output_dir, f"instance_segmentation_annotation_{timestamp}.json")
    
    return image_output_path, json_output_path, annotation_output_path

def main():
    """Main function to demonstrate instance segmentation."""
    logger.info("=== Starting Instance Segmentation Demo ===\n")

    # Paths (update these to your environment)
    model_path = r"python_demos\data\instance_segmentation_model.dwm"
    image_path =  r"python_demos\data\instance_segmentation_img.png"
    ...
    logger.info("=== Instance Segmentation Demo Completed ===")
