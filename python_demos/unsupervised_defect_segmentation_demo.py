...
def load_model(model_path, device=dlsdk.DeviceType.CPU):
    """
    Load the unsupervised defect segmentation model.
    ...
    try:
        ...
        model = dlsdk.UnsupervisedDefectSegmentation(model_path, device=device)
...
def run_inference(model, daoai_image, sensitivity_threshold=0.5):
    """
    Run inference on the image with error handling.
    
    Args:
        model: The unsupervised defect segmentation model object.
        daoai_image: The image in the format supported by the SDK.
        sensitivity_threshold (float): Confidence threshold for post-processing.
    
    Returns:
        prediction: Inference result, or None if an error occurs.
    """
    try:
        logger.info(f"Running inference with sensitivity threshold: {sensitivity_threshold}")
...
def print_detection_results(prediction, max_points_to_print=3):
    ...
    logger.info("\nMasks to Polygons:")
    polygons = prediction.mask.toPolygons()
    ...
    logger.info(f"Inference Decision: {prediction.decision}")
...
def generate_output_paths(base_dir=r"python_demos\output"):
    ...
    image_output_path = os.path.join(image_output_dir, f"unsupervised_defect_segmentation_result_{timestamp}.png")
    json_output_path = os.path.join(json_output_dir, f"unsupervised_defect_segmentation_prediction_{timestamp}.json")
    annotation_output_path = os.path.join(json_output_dir, f"unsupervised_defect_segmentation_annotation_{timestamp}.json")
    
    return image_output_path, json_output_path, annotation_output_path

def main():
    """Main function to demonstrate unsupervised defect segmentation."""
    ...
    logger.info("=== Unsupervised Defect Segmentation Demo Completed ===")
