...
def print_detection_results(prediction):
    """Print class labels and confidence for detection results, and the classification result with the highest confidence."""
    ...
    for i, flag in enumerate(prediction.flags):
        logger.info(f"  Detection {i + 1}:")
        logger.info(f"    Label: {flag.label}, Confidence: {flag.confidence:.2f}")
        
        # Track the highest confidence and corresponding label
        if highest_confidence is None or flag.confidence > highest_confidence:
            highest_confidence = flag.confidence
            classification_result = flag.label
    
    logger.info("\nClassification Result:")
    logger.info(f"  Label: {classification_result}, Confidence: {highest_confidence:.2f}")
...
def generate_output_paths(base_dir=r"python_demos\output"):
    ...
    image_output_path = os.path.join(image_output_dir, f"classification_result_{timestamp}.png")
    json_output_path = os.path.join(json_output_dir, f"classification_prediction_{timestamp}.json")
    annotation_output_path = os.path.join(json_output_dir, f"classification_annotation_{timestamp}.json")
    
    return image_output_path, json_output_path, annotation_output_path

def main():
    """Main function to demonstrate Classification."""
    ...
    logger.info("=== Classification Demo Completed ===")
